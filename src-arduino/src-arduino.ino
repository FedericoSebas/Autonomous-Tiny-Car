...
const char* mqttServer[3] = {"192.168.0.200","192.168.43.200", "z7a1aa9e-internet-facing-8896c3706ea4d887.elb.us-east-1.amazonaws.com"};
const int mqttPort = 1883;
const char* user[3] = {"","","CERTERO"};
const char* pass[3] = {"","","12345678"};
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(Topic == topic_motor_left){
    ...
    if(joyLeft[SPD].toInt())speed[LEFT] = map(joyLeft[SPD].toInt(),1,100,rc_minSpeed,255);
...
void setup() {
    ...
    */
    ...
    client1.setServer(mqttServer[localServer1], mqttPort);
    client2.setServer(mqttServer[localServer2], mqttPort);
    client3.setServer(mqttServer[cloudServer], mqttPort);
...
void loop() {
    ...
    if(state == RC){
        ...
        if(Forward[LEFT]) leftMotor.forward();
        if(Backward[LEFT]) leftMotor.backward();
        if(Forward[RIGHT]) rightMotor.forward();
        if(Backward[RIGHT]) rightMotor.backward();
        delay(1);
    }
