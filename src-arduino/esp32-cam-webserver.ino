...
const char* ssid = "Fede";
const char* password = "12345678";
const char* mqttServer[2] = {"192.168.0.200","192.168.43.200"};
const int mqttPort = 1883;

WiFiClient espClient;
PubSubClient client(espClient);

Motor leftMotor(15,13,2,0);
Motor rightMotor(12,16,14,1);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(Topic == topic_motor_autonomy_turn){
  ...
  if(Topic == topic_motor_left){
    ...
    if(joyLeft[SPD].toInt())speed[LEFT] = map(joyLeft[SPD].toInt(),0,100,rc_minSpeed,255);
    else speed[LEFT] = joyLeft[SPD].toInt();
  }
  if(Topic == topic_motor_right){
    ...
    if(joyRight[SPD].toInt())speed[RIGHT] = map(joyRight[SPD].toInt(),1,100,rc_minSpeed,255);
    else speed[RIGHT] = joyRight[SPD].toInt();
  }
  if(Topic == topic_state){
    incommingState = incommingMessage;
  }
  if(Topic == topic_motor_rc_speed){
    rc_minSpeed = incommingMessage.toInt();
  }
  if(Topic == topic_motor_autonomy_speed){
...
void reconnect(){
  while (!client.connected())
  {
    ...
    if (client.connect("CERTERO-MOTORS"))
    {
      ...
      client.subscribe(topic_motor_autonomy_turn);
      client.subscribe(topic_motor_rc_turn);
...
void setup() {
    ...
    */
    ...
    client.setServer(mqttServer[i], mqttPort);
...
void loop() {
    ...
    */
    ...
    client.loop();
    Forward = speed[LEFT] && speed[RIGHT] && deg[LEFT] >= 0 && deg[LEFT] <= 180 && deg[RIGHT] >= 0 && deg[RIGHT] <= 180;
    Backward = speed[LEFT] && speed[RIGHT] && deg[LEFT] >= 180 && deg[LEFT] <= 360 && deg[RIGHT] >= 180 && deg[RIGHT] <= 360;
    Left = speed[LEFT] && speed[RIGHT] && deg[LEFT] >= 0 && deg[LEFT] <= 180 && deg[RIGHT] >= 180 && deg[RIGHT] <= 360;
    Right = speed[LEFT] && speed[RIGHT] && deg[RIGHT] >= 0 && deg[RIGHT] <= 180 && deg[LEFT] >= 180 && deg[LEFT] <= 360;
    ...
    if(state == RC){
        ...
        if(Forward) twoMotors.Forward();
        if(Backward) twoMotors.Backward();
        if(Left) twoMotors.Left();
        if(Right) twoMotors.Right();
        delay(1);
    }
    if(state == AUTONOMY || state == RC_CONTROL) twoMotors.Move(turn,autonomy_speed);
