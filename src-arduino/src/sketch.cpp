...
int maxSpeed = 127;
int minSpeed = 0;
int levelLeftMotor = 1;
int levelRightMotor = 2;
double kp = 56;
double ki = -0.0005;
double kd = 1;
...
int speeds[3] = {minSpeed, normalSpeed, maxSpeed};

int autonomy_speed = 50;
...
const char *topic_motor_turn = "motor/turn";
const char *topic_motor_kp = "motor/kp";
const char *topic_motor_ki = "motor/ki";
const char *topic_motor_kd = "motor/kd";
const char *topic_motor_left = "motor/rc/left";
const char *topic_motor_right = "motor/rc/right";
const char *topic_motor_min_speed = "motor/min_speed";
const char *topic_motor_max_speed = "motor/max_speed";
const char *topic_motor_normal_speed = "motor/speed";
const char *topic_ota = "ota";
const char *topic_motor_level_left = "motor/left";
const char *topic_motor_level_right = "motor/right";
const char *topic_info = "motor/info";
const char *topic_display_left = "motor/display/left";
const char *topic_display_right = "motor/display/right";
//

//

// Declaration of motors
Motor leftMotor(12, 13, 1, 2);
Motor rightMotor(15, 14, 3, 4);
...
void CallbackTopic(char *topic, byte *message, unsigned int length)
{
    ...
    if (Topic == topic_ota)
    {
        isOTAEnabled = Message.toInt();
    }
    if (Topic == topic_motor_level_left)
    {
        levelLeftMotor = Message.toInt();
    }
    if (Topic == topic_motor_level_right)
    {
        levelRightMotor = Message.toInt();
    }
    if (Topic == topic_motor_turn)
    {
        turn = Message.toDouble();
    }

    if (Topic == topic_motor_kp)
    {
        kp = Message.toDouble();
    }

    if (Topic == topic_motor_ki)
    {
        ki = Message.toDouble();
    }

    if (Topic == topic_motor_kd)
    {
        kd = Message.toDouble();
    }
    ...
    {
        joyLeft[DEG] = Message.substring(0, Message.indexOf(" "));
        joyLeft[SPD] = Message.substring(Message.indexOf(" "), Message.length());
        joyLeft[DEG].trim();
        joyLeft[SPD].trim();
        deg[LEFT] = joyLeft[DEG].toInt();
        if (joyLeft[SPD].toInt())
            speed[LEFT] = round(mapFloat(joyLeft[SPD].toInt(), 1, 100, minSpeed, maxSpeed - levelLeftMotor));
        else
            speed[LEFT] = 0;
    }
    if (Topic == topic_motor_right)
    {
        joyRight[DEG] = Message.substring(0, Message.indexOf(" "));
        joyRight[SPD] = Message.substring(Message.indexOf(" "), Message.length());
        joyRight[DEG].trim();
        joyRight[SPD].trim();
        deg[RIGHT] = joyRight[DEG].toInt();
        if (joyRight[SPD].toInt())
            speed[RIGHT] = round(mapFloat(joyRight[SPD].toInt(), 1, 100, minSpeed, maxSpeed));
        else
            speed[RIGHT] = 0;
    }
    if (Topic == topic_state)
    {
        incommingState = Message;
    }
    if (Topic == topic_motor_min_speed)
    {
        minSpeed = Message.toInt();
    }
    if (Topic == topic_motor_max_speed)
    {
        maxSpeed = Message.toInt();
    }
    if (Topic == topic_motor_normal_speed)
    {
        normalSpeed = Message.toInt();
    }
}
...
void CallbackSubscribe()
{
    ...
    client.subscribe(topic_motor_kp);
    client.subscribe(topic_motor_ki);
    client.subscribe(topic_motor_kd);
    client.subscribe(topic_motor_left);
    client.subscribe(topic_motor_right);
    client.subscribe(topic_state);
    client.subscribe(topic_ota);
    client.subscribe(topic_motor_min_speed);
    client.subscribe(topic_motor_max_speed);
    client.subscribe(topic_motor_normal_speed);
    client.subscribe(topic_motor_level_left);
    client.subscribe(topic_motor_level_right);
}
...
void setup()
{
    ...
    if (strcmp(ssids[WIFI1], WiFi.SSID().c_str()) == 0)
    {
        client.setServer(servers[WIFI1], 1883);
    }

    else if (strcmp(ssids[WIFI2], WiFi.SSID().c_str()) == 0)
    {
        client.setServer(servers[WIFI2], 1883);
    }
    // connect in different ip servers depending of the ssid of the wifi selected
...
void loop()
{
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
        ...
        if (state == RC)
        {
            ...
            Forward[RIGHT] = deg[RIGHT] >= 0 && deg[RIGHT] <= 180;
            Backward[RIGHT] = deg[RIGHT] >= 180 && deg[RIGHT] <= 360;
            if (Forward[LEFT])
                leftMotor.forward();
            if (Backward[LEFT])
                leftMotor.backward();
        ...
        // Start PID System
        if (state == AUTONOMY || state == RC_CONTROL)
        {
            speeds[MIN] = minSpeed;
            speeds[NORMAL] = normalSpeed;
            speeds[MAX] = maxSpeed;
            // Autonomy PID
            String displayInfo = twoMotors.Move(turn, speeds, levelLeftMotor, levelRightMotor, kp, ki, kd);

            if (millis() - timeNow >= 10)
            {
                timeNow = millis();
                // add time to displayInfo
                displayInfo = String(timeNow) + displayInfo;
                client.publish(topic_info, displayInfo.c_str());
            }
        }
        // End PID System
    }
